...
public class PandoraSummon : ObjectPool
{
    ...
    public IEnumerator BeatA_SummonMobs()
    {
        for (int i = 0; i < PandoraController.instance.numberOfMob_; i++)
        {
            GameObject mob = GetObjectFromPool();
            if (mob != null)
            {
                mob.GetComponent<MobController>().LoadMobController();
                mob.transform.position = PandoraController.instance.summonPoint_.position;
                mob.SetActive(true);
            }
